 ...
 *
 * A photoreal three.js globe (day/normal/specular + cloud layer + fresnel atmosphere)
 * pushes in from orbit and rotates to frame North America; at peak zoom a white-out
 * flash masks a cut to a 2D Ontario map, whose border draws in and whose cities drop as
 * pins. Then the whole overlay lifts to reveal the homepage.
 *
 * Plays once per browser session (survives the language-switch reloads, which share the
 * session). Honors prefers-reduced-motion. Append ?intro=1 to force a replay.
 *
 * Texture URLs come from window.INTRO_TEX when present (the standalone file inlines them
 * as data URIs); otherwise they load from assets/. Geometry is embedded below. */
(function () {
  'use strict';
  ...
  var TEX = window.INTRO_TEX || {
    ...
    normal: 'assets/earth_normal.jpg', spec: 'assets/earth_spec.jpg'
  };
  ...
  // Load three.js only when the intro is actually going to run, so repeat visits
  // (and language-switch reloads) pay nothing for it.
  ...
  // No WebGL -> skip the intro entirely rather than risk a blank overlay.
  ...
  scene.add(clouds);
  var atmo = new THREE.Mesh(new THREE.SphereGeometry(1.19, 64, 64), new THREE.ShaderMaterial({
    vertexShader: 'varying vec3 vN;void main(){vN=normalize(normalMatrix*normal);gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}',
    fragmentShader: 'varying vec3 vN;void main(){float i=pow(0.70-dot(vN,vec3(0.,0.,1.)),2.6);gl_FragColor=vec4(0.32,0.60,1.0,1.0)*i;}',
    blending: THREE.AdditiveBlending, side: THREE.BackSide, transparent: true
  }));
  scene.add(atmo);
  scene.add(new THREE.AmbientLight(0x2b3a4d, 1.15));
  var sun = new THREE.DirectionalLight(0xfff6ec, 1.5);
  sun.position.set(-1.1, 0.6, 2.9);
  scene.add(sun);
  ...
  // animated state driven by the timeline
  var S = { camZ: 7.6, rotY: -8 * DEG - 2.5, rotX: 8 * DEG, cloudRotY: 0, stars: 0 };
  var running = true;
  function frame() {
    if (!running) return;
    earth.rotation.set(S.rotX, S.rotY, 0);
    clouds.rotation.set(S.rotX, S.rotY + S.cloudRotY, 0);
    atmo.rotation.set(S.rotX, S.rotY, 0);
    camera.position.z = S.camZ;
    stars.rotation.y = S.rotY * 0.15;
  ...
  var pinNodes = [];
  GEO.pins.forEach(function (p) {
    var g = document.createElementNS(SVGNS, 'g');
    g.setAttribute('transform', 'translate(' + p.x + ' ' + p.y + ')');
    var inner = document.createElementNS(SVGNS, 'g');
    inner.setAttribute('class', 'pin-inner');
    var ring = document.createElementNS(SVGNS, 'circle');
    ring.setAttribute('class', 'pin-ring'); ring.setAttribute('r', p.p ? 9 : 7);
    var dot = document.createElementNS(SVGNS, 'circle');
    dot.setAttribute('class', 'pin-dot' + (p.p ? ' is-primary' : '')); dot.setAttribute('r', p.p ? 8 : 5.5);
    var label = document.createElementNS(SVGNS, 'text');
    label.setAttribute('class', 'pin-label' + (p.p ? ' is-primary' : ''));
    label.setAttribute('x', p.p ? 15 : 11); label.setAttribute('y', p.p ? 6 : 4.5);
    label.textContent = p.n;
    inner.appendChild(ring); inner.appendChild(dot); inner.appendChild(label);
    g.appendChild(inner); pinsG.appendChild(g);
    pinNodes.push({ inner: inner, ring: ring, primary: p.p });
  });
  ...
  // Absolute backstop: the overlay is torn down no matter what — a stalled timeline,
  // a texture that never loads, a thrown error — so the page can never stay trapped.
  var watchdog = setTimeout(finish, 13500);

  var tl = gsap.timeline({ onComplete: finish });

  // Phase A — orbit, stars, first push-in
  tl.to(S, { stars: 1, duration: 1.3, ease: 'power1.out' }, 0);
  tl.to(S, { camZ: 4.7, duration: 2.7, ease: 'power1.inOut' }, 0);
  tl.to(S, { rotY: -8 * DEG - 0.85, rotX: 22 * DEG, duration: 2.7, ease: 'power1.inOut' }, 0);
  tl.to(S, { cloudRotY: 0.05, duration: 6, ease: 'none' }, 0);
  cap('Planet Earth', 0.8, 2.4);

  // Phase B — approach North America / Canada
  tl.to(S, { camZ: 2.5, duration: 2.6, ease: 'power2.in' }, 2.7);
  tl.to(S, { rotY: -6 * DEG, rotX: 31 * DEG, duration: 2.6, ease: 'power2.inOut' }, 2.7);
  cap('Canada', 3.0, 4.9);

  // Phase C — final dive + white-out cut to the map
  tl.to(S, { camZ: 1.5, duration: 1.1, ease: 'power3.in' }, 5.3);
  tl.to(flash, { opacity: 1, duration: 0.7, ease: 'power2.in' }, 5.5);
  tl.set(canvas, { autoAlpha: 0 }, 6.15);
  tl.set(mapEl, { autoAlpha: 1 }, 6.05);
  tl.to(flash, { opacity: 0, duration: 1.0, ease: 'power2.out' }, 6.15);
  cap('Ontario', 6.1, 8.0);

  // Phase D — border draws, context fades
  tl.fromTo(mapEl, { scale: 1.09, transformOrigin: '50% 44%' }, { scale: 1, duration: 1.8, ease: 'power2.out' }, 6.15);
  tl.to(neighborsPath, { opacity: 1, duration: 1.2, ease: 'power1.out' }, 6.3);
  tl.to(ontarioPath, { strokeDashoffset: 0, duration: 1.9, ease: 'power2.inOut' }, 6.35);

  // Phase E — pins drop (Toronto leads)
  var primary = pinNodes.filter(function (p) { return p.primary; });
  var rest = pinNodes.filter(function (p) { return !p.primary; });
  var ordered = primary.concat(rest);
  ordered.forEach(function (p, idx) {
    var at = 7.9 + idx * 0.16;
    tl.fromTo(p.inner, { opacity: 0, y: -26, scale: 0.5, transformOrigin: '50% 100%' },
      { opacity: 1, y: 0, scale: 1, duration: 0.62, ease: 'back.out(2.2)' }, at);
    tl.fromTo(p.ring, { attr: { r: p.primary ? 8 : 5.5 }, opacity: 0.9 },
      { attr: { r: p.primary ? 26 : 20 }, opacity: 0, duration: 1.1, ease: 'power2.out' }, at + 0.18);
  });

  // Phase F — lift away to reveal the site
  tl.to({}, { duration: 0.7 }, 9.9);   // hold
  tl.to(ov, { opacity: 0, duration: 0.95, ease: 'power2.inOut' }, 10.4);
  tl.to(mapEl, { scale: 1.06, duration: 0.95, ease: 'power2.in' }, 10.4);

  // debug hooks (replay flag only) — used to verify keyframes when WebGL screenshots fail
  if (FORCE) {
    window.__intro = {
      tl: tl, S: S, canvas: canvas, mapEl: mapEl,
      stopGL: function () { running = false; },
      seek: function (t) { tl.pause(t); },
      grab: function (q) { renderer.render(scene, camera); return canvas.toDataURL('image/jpeg', q || 0.7); }
    };
  }

  // self-driving frame capture for testing (?shot=<seconds>&out=<name>): pause the
  // timeline, stop the render loop, draw one frame, upload it — no external eval needed.
  var shot = /[?&]shot=([\d.]+)/.exec(location.search);
  var freeze = /[?&]freeze=([\d.]+)/.exec(location.search);
  if (shot) {   // globe frame -> upload canvas jpeg
    ...
    setTimeout(function () {
      ...
      requestAnimationFrame(function () {
        renderer.render(scene, camera); running = false;
        fetch('/' + out, { method: 'POST', body: canvas.toDataURL('image/jpeg', 0.72) });
      });
    }, 700);
  }
  if (freeze) {   // map phase -> pause + stop GL so a normal screenshot works
    var ft = parseFloat(freeze[1]);
    tl.pause();
    setTimeout(function () { tl.pause(ft); requestAnimationFrame(function () { renderer.render(scene, camera); running = false; }); }, 700);
  }
